...
- **Murthy** — daily progress, a daily summary note, and recurring daily
  ...
  stays private even though this repo is public. Also has an optional
  "Hey Murthy" wake-word voice assistant (mobile + desktop), built on the
  open-source openWakeWord engine run locally via ONNX Runtime — no
  account or cloud service involved. See `assets/murthy/onnx/README.md`
  for the one-time setup it needs (two generic model downloads + training
  a small custom keyword model, free, in a notebook), since none of that
